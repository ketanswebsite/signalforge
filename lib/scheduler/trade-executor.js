...
// The strategy's numbers: the target, the stop, the holding limit (GAPS #9). The drift guard's default below stays
// written out because tests/unit/strategy-version.test.js reads that line; it is StrategyParams.DEFAULT_ENTRY_DRIFT_PERCENT
...
class TradeExecutor {
    ...
    initialize() {
        ...
        cron.schedule('0 13 * * 1-5', async () => {
            ...
            return await this.executeMarketSignals('India');   // job_runs keeps what the run booked
        }, {
        ...
        cron.schedule('0 13 * * 1-5', async () => {
            ...
            return await this.executeMarketSignals('UK');
        }, {
        ...
        cron.schedule('0 13 * * 1-5', async () => {
            ...
            return await this.executeMarketSignals('US');
        }, {
        ...
        cron.schedule('0 0 * * *', async () => {
            ...
            return await this.cleanupOldPendingSignals();
        }, {
            timezone: "UTC"
        });
    ...
    async executeSingleSignal(signal, market) {
        try {
            // The house book: the admin's account (ADMIN_EMAIL, through config/admin.js)
            const userId = require('../../config/admin').adminEmail();
            ...
            // the 7 AM signal price, so the target and the stop mean what they say.
            const entryDate = new Date();
            const squareOffDate = new Date(entryDate);
            squareOffDate.setDate(squareOffDate.getDate() + StrategyParams.MAX_HOLDING_DAYS);
            const trade = {
                symbol: signal.symbol,
                entryDate: entryDate,
                entryPrice: executionPrice,
                targetPrice: StrategyParams.targetPrice(executionPrice),
                stopLossPercent: StrategyParams.STOP_LOSS_PERCENT,
                takeProfitPercent: StrategyParams.TAKE_PROFIT_PERCENT,
                ...
                for (const { signal, price } of eligible) {
                    ...
                    try {
                        ...
                        await TradeDB.insertTrade({
                            ...
                            entryDate: entryDate,
                            entryPrice: price,
                            targetPrice: StrategyParams.targetPrice(price),
                            stopLossPercent: StrategyParams.STOP_LOSS_PERCENT,
                            takeProfitPercent: StrategyParams.TAKE_PROFIT_PERCENT,
                            squareOffDate: squareOffDate.toISOString().split('T')[0],
                            status: 'active',
    ...
    async sendSubscriberNotification(chatId, market, booked) {
        try {
            // Every message sent here is counted as a 'booking-dm' (lib/telegram/delivery-counts.js)
            const telegram = require('../telegram/telegram-bot');
            const sendTelegramAlert = (to, alert) => telegram.sendTelegramAlert(to, { ...alert, kind: 'booking-dm' });
