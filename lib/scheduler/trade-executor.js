...
class TradeExecutor {
    ...
    async executeMarketSignals(market) {
        ...
        try {
            ...
            if (signals.length > 0) {
                console.log(`   📊 Signal dates in database:`);
                const signalDates = [...new Set(signals.map(s => new Date(s.signal_date).toISOString().split('T')[0]))];
                signalDates.forEach(date => {
                    const count = signals.filter(s => new Date(s.signal_date).toISOString().split('T')[0] === date).length;
                    console.log(`      ${date === today ? '✓' : '○'} ${date}: ${count} signals ${date === today ? '(TODAY - will execute)' : '(old - will skip)'}`);
                });
            }
