...
class TradeExecutor {
    ...
    async executeSingleSignal(signal, market) {
        try {
            // Get the first user from database (single user system)
            const usersResult = await TradeDB.pool.query('SELECT email FROM users LIMIT 1');
            const userId = usersResult.rows.length > 0 ? usersResult.rows[0].email : 'default';
