...
class TradeExecutor {
    ...
    async sendExecutionNotification(market, summary, results) {
        try {
            ...
            console.log(`   📤 [EXECUTOR] Sending execution notification to subscribers (${market}, ${summary.executed} trades)...`);
            const executionResults = await telegramBot.broadcastToSubscribers({
                type: 'custom',
                message
            }, 'execution');

            console.log(`   ✅ [EXECUTOR] Execution notification sent to ${executionResults.length} subscribers`);
