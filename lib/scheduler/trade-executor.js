    ...
    async sendExecutionNotification(market, summary, results) {
        try {
            ...
            if (!telegramBot || typeof telegramBot.broadcastToSubscribers !== 'function') {
                console.log('   ⚠️ [EXECUTOR] Telegram not configured - broadcastToSubscribers not available');
                return;
            }
            ...
            console.log(`   ✅ [EXECUTOR] Execution notification sent successfully`);
            console.log(`   📊 [EXECUTOR] Broadcast results: ${successCount} success, ${failedCount} failed (total: ${executionResults.length} subscribers)`);

            if (failedCount > 0) {
                console.log(`   ⚠️ [EXECUTOR] Some notifications failed to send:`);
                executionResults.filter(r => !r.success).forEach(r => {
                    console.log(`      • ${r.chatId}: ${r.error}`);
                });
            }

        } catch (error) {
            console.error(`   ❌ [EXECUTOR] Error sending Telegram notification: ${error.message}`);
