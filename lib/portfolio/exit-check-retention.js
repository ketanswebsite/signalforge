 ...
 *     you may find 4 days of minute data when you expected 30.
 *
 * The daily rollup is kept for ever either way, so this only decides how long
 * the minute-by-minute path survives. Whatever this returns is clamped by
 * resolveRetentionDays(): it can shorten the window down to
 * MIN_RETENTION_DAYS but never lengthen it past the configured value, and
 * anything that is not a number is ignored. Alert rows are out of reach
