 ...
 * minute. Apart from this job's rollup and the ops probe's counts, nothing
 * reads those routine rows: checkAlertSent() looks exclusively at
 * alert_sent = true rows — that is what stops an exit alert going out twice.
 *
...
// never from a caller. The one table pruned is the one rolled up.
const TABLES = ['trade_exit_checks'];
        ...
        for (const table of TABLES) {
            ...
            if (dryRun) {
                ...
                console.log(`🧹 [EXIT CHECKS] DRY RUN — ${table}: ${preview.wouldDelete} of ${stats.routineRows} routine rows ` +
                    `are older than ${retentionDays} full days and would be deleted, after rolling up ${preview.wouldRollUp} position-day(s)` +
            ...
            const rolledUp = await rollUpCompleteDays();
            ...
            console.log(`🧹 [EXIT CHECKS] ${table}: rolled up ${rolledUp} position-day(s); ` +
