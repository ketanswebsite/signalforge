...
class CapitalManager {
     ...
     * @param {string} userId - User email for capital isolation
     */
    async allocateForTrade(market, symbol, entryPrice, userId) {
        // Validate first
        const validation = await this.validateTradeEntry(market, symbol, userId);
        if (!validation.valid) {
     ...
     * ---------------------------
     * This function now expects camelCase field names from the transformation layer.
     * Defensive fallbacks to snake_case are provided for backward compatibility.
     *
     * Expected fields (camelCase):
     * - trade.investmentAmount (primary)
     * - trade.profitLoss (primary)
     * - trade.profitLossPercentage (primary)
     * - trade.tradeSize (fallback if investmentAmount missing)
     *
     * @param {string} userId - User email for capital isolation
     * @see database-postgres.js - FIELD NAME TRANSFORMATION LAYER DOCUMENTATION
     */
    async releaseFromTrade(trade, userId) {
        // Determine market from symbol if not provided
        const market = trade.market || this.getMarketFromSymbol(trade.symbol);

        // Get investment amount (camelCase primary, snake_case fallback for safety)
        const investmentAmount = trade.investmentAmount || trade.tradeSize ||
                                 trade.investment_amount || trade.trade_size || 0;

        // Use actual profitLoss if available (most accurate), otherwise calculate from percentage
        let plAmount;
        if (trade.profitLoss !== undefined && trade.profitLoss !== null) {
            // Primary: camelCase from transformation layer
            plAmount = parseFloat(trade.profitLoss);
        } else if (trade.profit_loss !== undefined && trade.profit_loss !== null) {
            // Defensive fallback: snake_case (for backward compatibility)
            console.warn(`[CAPITAL] WARNING: Received trade.profit_loss (snake_case) instead of trade.profitLoss (camelCase). Trade ID: ${trade.id}`);
            plAmount = parseFloat(trade.profit_loss);
        } else {
            // Calculate from percentage (camelCase primary, snake_case fallback)
            const plPercent = trade.profitLossPercentage || trade.profitLossPercent ||
                             trade.profit_loss_percentage || 0;
            plAmount = (investmentAmount * plPercent) / 100;
        }

        if (!investmentAmount) {
            // A zero release silently strands allocated capital and drops realized P/L —
            // this is the exact failure behind the ledger drift, so shout about it.
            console.error(`[CAPITAL] REFUSING zero-amount release for trade id=${trade.id} symbol=${trade.symbol}: no investmentAmount/tradeSize on the trade object. Ledger NOT updated — use TradeDB.closeTradeAndRelease or pass a fully-hydrated trade.`);
            return { released: 0, pl: 0, currency: trade.currency, market: market, skipped: true };
        }
        ...
        await TradeDB.releaseCapital(market, investmentAmount, plAmount, userId);
     ...
     * @param {string} userId - User email for capital isolation
     */
    async getCapitalSummary(userId) {
        const status = await this.getCapitalStatus(userId);
