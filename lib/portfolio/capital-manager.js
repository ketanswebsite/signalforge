...
class CapitalManager {
    constructor() {
        // Position sizes and limits: the strategy's (lib/shared/strategy-params.js)
        this.CONFIG = {
            TRADE_SIZES: StrategyParams.TRADE_SIZES,
            MAX_POSITIONS_TOTAL: StrategyParams.MAX_POSITIONS_TOTAL,
            MAX_POSITIONS_PER_MARKET: StrategyParams.MAX_POSITIONS_PER_MARKET
        };
    }
    ...
    calculateTradeSize(market, capital) {
        ...
        // Floor: never below MIN_TRADE_SIZE_SHARE of the standard size
        const minSize = this.CONFIG.TRADE_SIZES[market].amount * StrategyParams.MIN_TRADE_SIZE_SHARE;
    ...
    async validateTradeEntry(market, symbol, userId) {
        ...
        if (status.totals.totalPositions >= this.CONFIG.MAX_POSITIONS_TOTAL) {
            return {
                valid: false,
                reason: `Total portfolio limit reached (${status.totals.totalPositions}/${this.CONFIG.MAX_POSITIONS_TOTAL})`,
                code: 'TOTAL_LIMIT'
            };
        }
        ...
        if (marketCap.positions >= this.CONFIG.MAX_POSITIONS_PER_MARKET) {
            return {
                valid: false,
                reason: `Market limit reached for ${market} (${marketCap.positions}/${this.CONFIG.MAX_POSITIONS_PER_MARKET})`,
                code: 'MARKET_LIMIT'
            };
        }
