...
class ExitMonitor {
    ...
    async notifyOwner(message) {
        ...
        const chatId = CloseFailures.rememberOwnerChatId(await TradeDB.getUserChatId(adminEmail));
        if (!chatId) {
            ...
            console.error(`❌ [EXIT MONITOR] ${adminEmail} has no linked Telegram — the owner alert could not be sent`);
            return false;
        }
        ...
        return await telegramBot.sendTelegramAlert(chatId, { type: 'custom', message: CloseFailures.plainText(message) }) === true;
    }
