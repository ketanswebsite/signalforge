...
// Yahoo in process: the live price no longer comes back through this server's own /yahoo/quote
...
class ExitMonitor {
    ...
    async checkTradeExit(trade) {
        try {
            ...
            if (!closedTrade) {
                // Nothing left to close: the trade was sold or deleted elsewhere
                // after this pass read it (closeTrade has logged that). A close the
                // database refuses throws into the catch above instead. This pass
                // closed nothing, so it sends no alert
                await this.recordExitCheck(trade.id, currentPrice, plPercent, holdingDays, false, exitType);
                return { shouldExit: false, closedElsewhere: true };
            }
    ...
    async fetchCurrentPrice(symbol) {
        try {
            // Yahoo in process (lib/shared/yahoo-client.js): the one-day chart GET /yahoo/quote
            // serves, without an HTTP call back into this server
            const data = await YahooClient.fetchQuoteChart(symbol, { timeout: 20000 });
    ...
    async sendExitAlert(trade, exitPrice, plPercent, exitType, exitReason) {
        try {
            ...
            const adminEmail = require('../../config/admin').adminEmail();
            if (ownerId === adminEmail || ownerId === 'default') {
                await telegramBot.broadcastToSubscribers({ ...alertMessage, kind: 'exit' }, 'all');
                console.log(`✅ [EXIT MONITOR] Broadcast alert sent for ${trade.symbol}`);
            } else {
                // The owner DM — and only the DM — answers to their Alerts page
                // switches. The position is closed either way, and the public
                // broadcast above never consults anyone's preferences.
                ...
                } else if (await AlertPolicy.ownerWantsAlert(() => TradeDB.getAlertPreferences(ownerId), exitType)) {
                    const delivered = await telegramBot.sendTelegramAlert(chatId, { ...alertMessage, kind: 'exit-dm' });
                    console.log(delivered
                        ? `✅ [EXIT MONITOR] Owner DM sent for ${trade.symbol} → ${ownerId}`
                        : `⚠️ [EXIT MONITOR] Owner DM for ${trade.symbol} → ${ownerId} not delivered`);
                } else {
                    console.log(`ℹ [EXIT MONITOR] ${ownerId} has ${exitType} alerts switched off — exit DM for ${trade.symbol} not sent`);
                }
            }
        } catch (error) {
