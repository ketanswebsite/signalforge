...
class ExitMonitor {
    ...
    initialize() {
        ...
        const checkJob = cron.schedule('*/5 * * * *', async () => {
            ...
            // Only run during market hours (weekdays, 6 AM - 9 PM UK time)
        ...
        console.log('📅 [EXIT MONITOR] Checking every 5 minutes during market hours (6 AM - 9 PM UK time)\n');
    }
    ...
    async checkTradeExit(trade) {
        try {
            ...
            const closedTrade = await this.closeTrade(trade, currentPrice, plPercent, exitType, exitReason);
