 ...
 * this module too, under namespaced('hc'), but writes no evidence rows: its
 * exit-check table does not exist on production, so for those positions the
 * log line and the owner's message are the record.
 *
 ...
 * trade, and once a high-conviction pass (every 10 minutes) per stuck
 * high-conviction position — and every `true` is a Telegram message on your
 * phone. The tension: a position sitting past its stop is worth interrupting
 * you for, but an alarm that nags gets muted, and a muted alarm is how the
 * same-day exit bug went unseen in the first place.
 *
 ...
 *   - namespace: a stuck high-conviction position ('hc') is also holding back
 *     the exit broadcast every subscriber is waiting for. Note that its
 *     attempts arrive ten minutes apart, not one.
 *
 ...
 */
async function notifyOwner(message, { TradeDB, telegramBot }) {
