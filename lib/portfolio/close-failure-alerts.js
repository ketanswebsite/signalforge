...
// The bot sends legacy Markdown: dynamic text goes through markdownSafe(), and plainText() is the retry
const { markdownSafe, plainText } = require('../telegram/markdown');
...
async function deliverToOwner(message, { TradeDB, telegramBot }) {
    ...
    if (await telegramBot.sendTelegramAlert(chatId, { type: 'custom', message, kind: 'owner-alert' })) {
        return true;
    }
    ...
    return await telegramBot.sendTelegramAlert(chatId, { type: 'custom', message: plainText(message), kind: 'owner-alert' }) === true;
}
