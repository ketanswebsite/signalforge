    ...
    async sendEODSummary() {
        ...
        try {
            ...
            for (const chunk of chunks) {
                await broadcastToSubscribers({ type: 'custom', message: chunk, kind: 'eod' }, 'portfolio');
            }
            ...
            for (const sub of subscribers) {
                ...
                try {
                    ...
                    if (dm.length <= 3900) {
                        await sendTelegramAlert(sub.telegram_chat_id, { type: 'custom', message: dm, kind: 'eod-dm' });
                    } else {
                        let part = '';
                        for (const line of dm.split('\n')) {
                            if ((part + line).length > 3800) {
                                await sendTelegramAlert(sub.telegram_chat_id, { type: 'custom', message: part.trimEnd(), kind: 'eod-dm' });
                                part = '';
                            }
                            part += line + '\n';
                        }
