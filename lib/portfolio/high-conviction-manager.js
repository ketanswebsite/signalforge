...
// Dated exchange rates (GAPS #11): the P&L in the other two currencies is converted at the rate of its own day
const FxRates = require('../shared/fx-rates');
...
// The strategy's numbers: this book's investment per position, its target, its stop, its holding limit (GAPS #9)
const StrategyParams = require('../shared/strategy-params');

// Investment per position, per market (StrategyParams.HIGH_CONVICTION_INVESTMENTS)
const DEFAULT_INVESTMENTS = StrategyParams.HIGH_CONVICTION_INVESTMENTS;

// GAPS #15 in the weekly report: a closed position's market index over the same days (benchmark_return_percent,
// filled nightly by lib/portfolio/benchmark-fill.js) and the position's lead over it, in percentage points
const INDEX_NAMES = { '^NSEI': 'NIFTY 50', '^FTSE': 'FTSE 100', '^GSPC': 'S&P 500' };
const signed = value => `${value >= 0 ? '+' : ''}${value.toFixed(2)}`;
const withBenchmark = trade => Number.isFinite(parseFloat(trade.benchmark_return_percent)) && Number.isFinite(parseFloat(trade.pl_percent));

/** "vs FTSE 100: +1.20% (+3.30 pts)" for a closed position whose index return is known, else null */
function benchmarkLine(trade) {
    if (!withBenchmark(trade)) return null;
    const index = parseFloat(trade.benchmark_return_percent);
    const own = parseFloat(trade.pl_percent);
    return `vs ${INDEX_NAMES[trade.benchmark_symbol] || 'its index'}: ${signed(index)}% (${signed(own - index)} pts)`;
}

/** The week's alpha: the closed positions' average return against their indexes' average, over those with one */
function weekAlphaLine(trades) {
    const known = trades.filter(withBenchmark);
    if (known.length === 0) return null;
    const mean = values => values.reduce((sum, v) => sum + v, 0) / values.length;
    const own = mean(known.map(t => parseFloat(t.pl_percent)));
    const index = mean(known.map(t => parseFloat(t.benchmark_return_percent)));
    return `Alpha this week: ${signed(own - index)} pts (average ${signed(own)}% against the index's ${signed(index)}%, ${known.length} of ${trades.length} closed)`;
}
...
class HighConvictionPortfolioManager {
    ...
    /**
     * Calculate P&L in all currencies: in the market's own currency it is the P&L itself; the other two are it at
     * the dated rates of `day` (lib/shared/fx-rates.js: the last daily close on or before it). The fixed
     * EXCHANGE_RATES apply only while no dated rate is stored at all.
     * @param {string} [day]  YYYY-MM-DD, the day of the price: today for the 10-minute pass and a manual close
     */
    calculatePL(entryPrice, currentPrice, shares, market, day = new Date().toISOString().split('T')[0]) {
        ...
        // Convert to all three currencies at the day's dated rates (GAPS #11)
        const rates = FxRates.ratesOn(day) || EXCHANGE_RATES;
        let plGBP, plINR, plUSD;

        if (market === 'UK') {
            plGBP = nativePL;
            plINR = nativePL * rates.GBP_TO_INR;
            plUSD = nativePL * rates.GBP_TO_USD;
        } else if (market === 'India') {
            plINR = nativePL;
            plGBP = nativePL * rates.INR_TO_GBP;
            plUSD = nativePL * rates.INR_TO_USD;
        } else { // US
            plUSD = nativePL;
            plGBP = nativePL * rates.USD_TO_GBP;
            plINR = nativePL * rates.USD_TO_INR;
        }
    ...
    async addTradeFromScan(opportunity) {
        try {
            ...
            const targetPrice = StrategyParams.targetPrice(entryPrice); // the target, from the actual entry
            const stopLossPrice = StrategyParams.stopLossPrice(entryPrice); // the stop, from the actual entry

            // Calculate square off date (the holding limit from entry)
            const entryDate = new Date(trade.signalDate || trade.entryDate);
            const squareOffDate = new Date(entryDate);
            squareOffDate.setDate(entryDate.getDate() + StrategyParams.MAX_HOLDING_DAYS);
    ...
    async updateAllActiveTrades() {
        ...
        try {
            ...
            // The dated exchange rates calculatePL converts at (GAPS #11), read from the store at most hourly
            ...
            for (const trade of activeTrades) {
                ...
                if (pl.plPercent >= StrategyParams.TAKE_PROFIT_PERCENT) {
    ...
    formatWeeklyReportMessage(data) {
        ...
        if (closedThisWeek.length > 0) {
            ...
            closedThisWeek.forEach((trade, index) => {
                ...
                const vsIndex = benchmarkLine(trade);
                if (vsIndex) message += `   ${vsIndex}\n`;
            });
            const alpha = weekAlphaLine(closedThisWeek);
            if (alpha) message += `${alpha}\n`;
            message += `\n`;
        }
