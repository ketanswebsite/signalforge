...
function initializeTelegramBot() {
  ...
  if (bot && !isProduction) {
    console.log('🧹 Clearing any existing webhooks for development mode...');
    bot.deleteWebHook().catch(err => console.log('Note: No webhook to clear'));
  }
  ...
  if (isProduction && process.env.RENDER_EXTERNAL_URL) {
    ...
    console.warn('⚠️ Production mode but no RENDER_EXTERNAL_URL - webhook not set');
  }
...
function processUpdate(update) {
  if (!bot) {
    console.error('❌ Bot not initialized - cannot process update');
    return;
  }
