        ...
        try {
            ...
            if (alertOpportunities.length > 0) {
                ...
                if (!storageResult.success || storageResult.created === 0) {
                    const errorMsg = `⚠️ Signal storage failed! Found ${alertOpportunities.length} signals but only stored ${storageResult.created || 0}. Errors: ${storageResult.errors || 0}`;
                    console.error(errorMsg);

                    // Throw error to prevent Telegram alerts being sent when signals weren't stored
                    throw new Error(`Signal storage failed - execution will have no signals to process. Created: ${storageResult.created || 0}, Errors: ${storageResult.errors || 0}, Message: ${storageResult.error || 'Unknown error'}`);
                }

                console.log(`✅ [SIGNALS] Storage validated: ${storageResult.created} signals stored successfully`);
