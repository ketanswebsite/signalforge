...
class StockScanner {
    ...
    initialize() {
        ...
        // Weekend AI conviction sweep: Saturday 8 AM UK, scores the full
        // universe once; the verdicts serve the whole following week's
        // trades and simulations. Disable with CONVICTION_SWEEP=false.
        if (process.env.CONVICTION_SWEEP !== 'false') {
            const sweepJob = cron.schedule('0 8 * * 6', async () => {
                console.log('🧠 [CRON] Saturday AI conviction sweep starting...');
                try {
                    const { runConvictionSweep } = require('../../ml/conviction-sweep');
                    await runConvictionSweep();
                } catch (error) {
            ...
            console.log('🧠 [SCANNER] Weekend AI sweep scheduled: Saturday 8:00 AM UK (full universe, verdicts reused all week)');
        }
    ...
    formatOpportunitiesMessage(opportunities) {
        ...
        message += `🕐 Scan Time: ${formatDateTimeUK(new Date())}`;
