...
class StockScanner {
    ...
    getComprehensiveStockList() {
        // Use the existing backend data as base
        const { getStockLists } = require('../shared/stock-data');
        const lists = getStockLists();
        
        // For now, return backend stocks but log the limitation
        // TODO: Implement comprehensive frontend data import
        const backendCount = [
            ...lists.nifty50,
            ...lists.niftyNext50,
        ...
        console.log(`⚠️ Note: Backend currently has ${backendCount.length} stocks vs frontend's 2000+`);
        console.log(`📊 Breakdown: niftyMidcap150=${lists.niftyMidcap150.length} (vs 537), ftse250=${lists.ftse250.length} (vs 209), usStocks=${lists.usStocks.length} (vs 1101)`);
    ...
    async fetchStockData(symbol) {
        try {
            ...
            // Use same relative URL format as frontend
            const proxyUrl = `/yahoo/history?symbol=${symbol}&period1=${startDate}&period2=${endDate}&interval=1d`;
            
            // Use fetch (requires node-fetch for Node.js) instead of axios with baseURL
