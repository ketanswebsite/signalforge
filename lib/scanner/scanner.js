...
class StockScanner {
    ...
    initialize() {
        ...
        // The cron fires every Saturday; the date guard keeps only the first
        // one — explicit, rather than relying on cron day-of-month/day-of-week
        // combination semantics. Disable with CONVICTION_SWEEP=false.
        if (process.env.CONVICTION_SWEEP !== 'false') {
            const sweepJob = cron.schedule('0 8 * * 6', async () => {
                const ukDay = new Date(new Date().toLocaleString("en-US", {timeZone: "Europe/London"})).getDate();
                if (ukDay > 7) {
        ...
        // per-minute inserts. A dry run (logs only) unless EXIT_CHECK_PRUNE=true.
