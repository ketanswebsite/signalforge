    ...
    async storeSignalsForExecution(opportunities) {
        try {
            ...
            if (response.status === 200) {
                ...
                console.log(`   ✓ Signals stored: ${created} created, ${duplicates} duplicates, ${errors} errors`);
                return { success: true, created, duplicates, errors };
            } else {
                console.error('   ✗ Failed to store signals:', response.statusText);
                return { success: false, error: response.statusText };
            }

        } catch (error) {
            console.error('   ✗ Error storing signals:', error.message);
