...
class StockScanner {
    ...
    async runHighConvictionScan(chatId = null) {
        ...
        try {
            ...
            const recentOpportunities = highConvictionOpportunities.filter(opp => {
                ...
                const isRecent = FrontendBacktestCalculator.calculateDaysBetween(signalDate, new Date()) <= 2;
    ...
    async findHighConvictionOpportunities(allStocks) {
        ...
        for (let i = 0; i < allStocks.length; i += batchSize) {
            ...
            console.log(`🔍 Batch results for filtering:`);
            batchResults.forEach(opp => {
                if (opp && opp.trade) {
            ...
            console.log(`📊 Filtered ${batchResults.length} → ${highConvictionBatch.length} high conviction opportunities`);
        ...
        // Debug: Log final opportunities win rates
        console.log(`🎯 Final opportunities summary:`);
        opportunities.forEach(opp => {
            const winRate = opp.trade?.winRate || 0;
            console.log(`  ${opp.stock.symbol}: ${winRate.toFixed(1)}% win rate`);
        });
    ...
    formatHighConvictionMessage(alertOpportunities, totalOpportunities) {
        ...
        alertOpportunities.forEach((opp, index) => {
            ...
            console.log(`📤 Sending to telegram: ${opp.stock.symbol} with ${winRate.toFixed(1)}% win rate`);
