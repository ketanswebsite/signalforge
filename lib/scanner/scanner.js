    ...
    initialize() {
        ...
        const dailyScanJob = cron.schedule('0 7 * * 1-5', async () => {
            ...
            try {
                ...
                return result;   // job_runs keeps what the run did
            } catch (error) {
        ...
        if (process.env.CONVICTION_SWEEP !== 'false') {
            const sweepJob = cron.schedule('0 8 * * 6', async () => {
                try {
                    ...
                    if (!isSweepDay()) {
                        ...
                        return { skipped: 'not the first Saturday of the month' };
                    }
                    console.log('🧠 [CRON] First Saturday of the month — monthly AI conviction sweep starting...');
                    return await runConvictionSweep({ trigger: 'monthly' });
                } catch (error) {
            ...
            const sweepWatchdogJob = cron.schedule('*/30 9-20 * * 6', async () => {
                try {
                    return await require('../../ml/conviction-sweep').runSweepWatchdog();
                } catch (error) {
        ...
        const ledgerDriftJob = cron.schedule('30 22 * * 1-5', async () => {
            try {
                return await require('../portfolio/ledger-drift-check').runLedgerDriftCheck();
            } catch (error) {
