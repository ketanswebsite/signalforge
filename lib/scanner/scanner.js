    ...
    async runHighConvictionScan(chatId = null) {
        ...
        try {
            ...
            if (alertOpportunities.length > 0) {
                ...
                await this.storeSignalsForExecution(alertOpportunities);
    ...
    async storeSignalsForExecution(opportunities) {
        try {
            ...
            const signalsToStore = opportunities.map(opp => {
                ...
                // Log each signal being stored
                console.log(`   📝 Signal: ${symbol} (${market}) - Entry: ${entryPrice}, Win Rate: ${opp.trade?.winRate || 0}%, Signal Date: ${signalDate}`);
            ...
            if (response.status === 200) {
                ...
                console.log(`✅ [SIGNALS] Signals stored successfully: ${created} created, ${duplicates} duplicates, ${errors} errors`);
                if (created > 0) {
                    console.log(`✅ [SIGNALS] ${created} new signals will be auto-executed at 1 PM market time`);
                }
                if (duplicates > 0) {
                    console.log(`⚠️ [SIGNALS] ${duplicates} signals already existed (duplicates skipped)`);
                }
                if (errors > 0) {
                    console.error(`❌ [SIGNALS] ${errors} signals failed to store`);
                }
                return { success: true, created, duplicates, errors };
            } else {
                console.error(`❌ [SIGNALS] Failed to store signals: ${response.statusText}`);
                console.error(`❌ [SIGNALS] Response status: ${response.status}`);
            ...
            if (error.response) {
                console.error(`❌ [SIGNALS] API Response: ${error.response.status} - ${error.response.data}`);
            }
