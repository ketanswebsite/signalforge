...
class StockScanner {
    ...
    async runHighConvictionScan(chatId = null) {
        ...
        try {
            ...
            // Run real backtesting on all stocks to find high conviction opportunities
            const highConvictionOpportunities = await this.findAllDTIOpportunities(allStocks);
            console.log(`⭐ High conviction opportunities found (>75% win rate): ${highConvictionOpportunities.length}`);
    ...
    async scanStockBatchWithBacktest(stocks) {
        ...
        const promises = stocks.map(async (stock) => {
            try {
                ...
                if (opportunity && opportunity.winRate > 75) {  // Only high conviction (>75% win rate)
                    return {
                        stock: {
                            symbol: stock.symbol,
                            name: stock.name
