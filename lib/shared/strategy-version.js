 ...
 * Every number in strategy-params.js is one of these, so changing any of them is a bump.
...
// The entry drift guard's default (strategy-params.js). The executor and the high-conviction book read
// MAX_ENTRY_DRIFT_PERCENT the same way; tests/unit/strategy-version.test.js checks both of their lines
const DEFAULT_ENTRY_DRIFT_PERCENT = StrategyParams.DEFAULT_ENTRY_DRIFT_PERCENT;
...
const SWITCHES = [
    ...
    ['drift', env => {
        const percent = StrategyParams.entryDriftPercent(env);
