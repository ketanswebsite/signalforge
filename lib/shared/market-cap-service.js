 ...
 * `delayMs` apart. A request that fails (after the client's one retry on a 401) counts its
 * symbols as failed; three in a row stop the run. Never throws for Yahoo's sake. Ends with one
 ...
 * @returns {Promise<object>} the counts, requests and failedRequests, byMarket
 *   ({India|UK|US: {requested, updated}}) and error (the last failure's message, or null)
 */
async function updateMarketCaps(symbols, batchSize = YahooClient.QUOTE_BATCH_MAX, delayMs = 1000) {
    ...
    const results = {
        ...
        requests: 0, failedRequests: 0, byMarket: {}, error: null
    };
    ...
    for (let i = 0; i < symbols.length; i += size) {
        ...
        let quotes;
        try {
            quotes = await YahooClient.fetchQuotes(batch);
            failedInARow = 0;
        } catch (error) {
