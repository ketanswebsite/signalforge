...
// required in node
const DTICalcParams = (typeof window !== 'undefined' && window.StrategyParams)
...
function aggregateTo7Day(dates, high, low) {
    ...
    for (let i = 0; i < dates.length; i++) {
        ...
        // We start a new period every SEVEN_DAY_DTI_BARS bars, counted from the first bar
        if (i % DTICalcParams.SEVEN_DAY_DTI_BARS === 0 && i > 0) {
 ...
 * @param {number} entryThreshold - DTI threshold for trade entry (default -40, Blau's value; the live rules use StrategyParams.ENTRY_THRESHOLD, GAPS #4)
...
function analyzeStock(stockData, params = {}) {
    const {
        r = DTICalcParams.DTI_PERIODS.r,
        s = DTICalcParams.DTI_PERIODS.s,
        u = DTICalcParams.DTI_PERIODS.u,
        entryThreshold = -40
    } = params;
