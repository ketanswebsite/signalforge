...
function detectTradeSignals(dti, daily7DayDTI, entryThreshold = -40) {
    ...
    for (let i = 1; i < dti.length; i++) {
        ...
        // Entry signal: DTI below threshold, trending upward, AND 7-day DTI is positive (matches frontend)
        if (currentDTI < entryThreshold && currentDTI > prevDTI && current7DayDTI > 0) {
            signals.push({
                index: i,
                type: 'entry',
                dti: currentDTI,
...
function analyzeStock(stockData, params = {}) {
    ...
    const isOpportunity = currentDTI < entryThreshold && currentDTI > previousDTI && current7DayDTI > 0;
