...
function capRows(text) {
    ...
    // Judged on the text with its quoted strings, quoted names and comments blanked out: a semicolon left
    // over means a second statement, which gets no LIMIT (Postgres refuses the text as multiple commands,
    // rather than with a syntax error at an added LIMIT); a "limit" inside a string or comment is no LIMIT
    const code = statement.replace(/'(?:[^']|'')*'|"(?:[^"]|"")*"|--[^\n]*|\/\*[\s\S]*?\*\//g, ' ');
    return /^select\b/i.test(code.trim()) && !/\blimit\b/i.test(code) && !code.includes(';')
        ? `${statement}\nLIMIT ${ROW_CAP}`
        : statement;
}
