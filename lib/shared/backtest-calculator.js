...
// The strategy's parameters: window.StrategyParams in a page (the page loads strategy-params.js first),
// required in node
const BacktestCalcParams = (typeof window !== 'undefined' && window.StrategyParams)
...
function runBacktest(stockData, params = {}) {
    const {
        r = BacktestCalcParams.DTI_PERIODS.r,
        s = BacktestCalcParams.DTI_PERIODS.s,
        u = BacktestCalcParams.DTI_PERIODS.u,
        entryThreshold = BacktestCalcParams.ENTRY_THRESHOLD,
        takeProfitPercent = BacktestCalcParams.TAKE_PROFIT_PERCENT,
        stopLossPercent = BacktestCalcParams.STOP_LOSS_PERCENT,
        maxHoldingDays = 30
    } = params;
    ...
    // The last bar the loop judged. A bar without a close (Yahoo sends null, which the pages
    // parse as NaN) has no price to trade at, so it opens and closes nothing: an open trade is
    // judged again on the next bar with a close (a time exit on such a bar booked a NaN P/L)
    let lastJudgedIndex = 0;
    ...
    for (let i = 1; i < stockData.dates.length; i++) {
        ...
        // Against the last bar judged, so a 7-day turn on a skipped bar still exits at the next close
        const prev7DayDTI = sevenDayDTIData.daily7DayDTI[lastJudgedIndex];
        if (!(currentPrice > 0)) continue;
        lastJudgedIndex = i;

        // Check for entry signal
        if (!currentTrade) {
            // Entry conditions: DTI below threshold, trending upward, AND 7-day DTI trending upward (matches frontend)
            const prev7DayDTI = i > 0 ? sevenDayDTIData.daily7DayDTI[i - 1] : null;
    ...
    // Add any open trade (or force-close if held too long), as of the last bar that had a close
    if (currentTrade) {
        const lastIndex = lastJudgedIndex;
        const lastPrice = stockData.close[lastIndex];
        const holdingDays = calculateDaysBetween(currentTrade.entryDate, stockData.dates[lastIndex]);
        const plPercent = ((lastPrice - currentTrade.entryPrice) / currentTrade.entryPrice) * 100;

        // Force-close if held for >= maxHoldingDays
        const shouldForceClose = holdingDays >= maxHoldingDays;
        ...
        trades.push({
            ...
            plPercent: plPercent,
            holdingDays: holdingDays,
            exitReason: shouldForceClose ? 'Max Days' : 'Open',
            isWin: plPercent > 0,
            isOpen: !shouldForceClose
        });
    }
...
function checkForOpportunity(stockData, params = {}) {
    ...
    // Check for entry signal (matches frontend anticipatory logic)
    const prev7DayDTI = lastIndex > 0 ? backtest.sevenDayDTI.daily7DayDTI[lastIndex - 1] : null;
    const sevenDayConditionMet = prev7DayDTI !== null ? current7DayDTI > prev7DayDTI : true;
 ...
 */
function isWithinTradingDays(signalDate, daysToCheck = BacktestCalcParams.RECENT_SIGNAL_TRADING_DAYS, currentDate = new Date()) {
