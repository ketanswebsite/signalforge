...
class PgSessionStore extends session.Store {
    ...
    /**
     * End every session of one account (by the signed-in user's email), on every device. Account deletion
     * needs it: a session left behind would re-create the users row on its next request.
     */
    destroyUserSessions(email, callback) {
        this.run(async () => {
            const { rowCount } = await this.pool.query(
                `DELETE FROM ${TABLE} WHERE sess->'passport'->'user'->>'email' = $1`, [email]);
            return rowCount;
        }, callback);
    }
