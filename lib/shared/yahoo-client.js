 ...
 *                                      (the exit monitor, the 1 PM executor, POST /api/prices)
...
// query1 first, then query2: one host can refuse a network the other still serves
const CRUMB_URLS = [CRUMB_URL, 'https://query2.finance.yahoo.com/v1/test/getcrumb'];
...
// The last handshake, step by step (POST /api/ops/yahoo-check reports it): Yahoo throttles some networks' handshakes
// with 429s while the chart endpoint still answers them (the server's 06:00 market-cap refresh, 2026-09-25)
let lastHandshake = null;

/** An error for a handshake step Yahoo refused; its status sits where axios puts one (error.response.status). */
function stepError(step, status, words) {
  const error = new Error(`${words} (the ${step} request answered ${status || 'nothing'})`);
  error.yahooStep = step;
...
async function getSession() {
  ...
  const handshake = { at: new Date().toISOString(), cookie: null, crumb: [] };
  ...
  handshake.cookie = { status: probe.status, names: setCookie.map(c => c.split('=')[0]) };
  if (!cookie) throw stepError('cookie', probe.status, 'No Yahoo cookie');
  for (const url of CRUMB_URLS) {
    const host = new URL(url).host;
    let crumbRes;
    try {
      crumbRes = await axios.get(url, {
        headers: { 'User-Agent': USER_AGENT, 'Cookie': cookie },
        timeout: 15000
      });
    } catch (error) {
      const status = Number(error && error.response && error.response.status) || null;
      handshake.crumb.push({ host, status, ok: false });
      if (!status) throw error;                             // a network failure: as it came
      if (url !== CRUMB_URLS[CRUMB_URLS.length - 1]) continue;  // an HTTP refusal (a 429 above all): the other host
      throw stepError('crumb', status, 'No Yahoo crumb from query1 or query2');
    }
    const crumb = typeof crumbRes.data === 'string' ? crumbRes.data.trim() : '';
    const ok = Boolean(crumb) && !crumb.includes('<');
    handshake.crumb.push({ host, status: crumbRes.status || null, ok });
    if (!ok) throw stepError('crumb', crumbRes.status, 'No Yahoo crumb');
...
/**
 * One fresh handshake and one quote, step by step: what Yahoo answers this server right now (POST
 * /api/ops/yahoo-check). Never throws. At most seven requests: the cookie, two crumbs, one quote, and one more
 * handshake and quote if the quote answers 401.
 * @param {string[]} [symbols]
 * @returns {Promise<{ok: boolean, handshake: object|null, quote: object|null, error: object|null, ms: number}>}
 */
async function checkSession(symbols = ['AAPL', 'VOD.L', 'RELIANCE.NS']) {
  const started = Date.now();
  invalidateSession();
  const report = { ok: false, handshake: null, quote: null, error: null, ms: 0 };
  try {
    const quotes = await fetchQuotes(symbols);
    report.quote = { status: 200, caps: Object.fromEntries(quotes.map(q => [q.symbol, typeof q.marketCap === 'number' ? q.marketCap : null])) };
    report.ok = true;
  } catch (error) {
    const status = Number(error && error.response && error.response.status) || null;
    report.error = { step: (error && error.yahooStep) || null, status, message: String((error && error.message) || error).slice(0, 200) };
    if (error && error.yahooStep === 'quote') report.quote = { status };
  }
  report.handshake = lastHandshake;
...
module.exports = {
  ...
  fetchQuotes,
  checkSession,
  getLastHandshake: () => lastHandshake,
  QUOTE_BATCH_MAX,
  USER_AGENT
};
