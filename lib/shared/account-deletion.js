 ...
 * route deletes it. The admin guard and the executor name the same account (config/admin.js).
 */
 ...
 * @param {object} [opts.stripe]      a Stripe client (tests); by default config/stripe.js's
 * @returns {Promise<{found: boolean, financialRecordsRetained: boolean}>}
 *   Rejects with code 'PAYMENT_PROVIDER', having deleted nothing, when a paid plan Stripe still renews
 *   could not be ended in Stripe.
 */
async function deleteAccount({ pool, email, requestedBy, ipAddress = null, byAdmin = false, stripe = null }) {
    ...
    try {
        ...
        // A paid plan Stripe still renews would go on charging a deleted account: it is ended in Stripe first.
        // When Stripe cannot be told, this throws and the transaction deletes nothing.
        for (const subscription of financialRecords.subscriptions) {
            const stripeSubscription = StripeBilling.stripeSubscriptionOf(subscription);
            if (stripeSubscription && subscription.status === 'active') {
                await StripeBilling.tellStripe(stripeSubscription, 'end-now', { stripe });
            }
        }
