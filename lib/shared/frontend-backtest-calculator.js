...
// The strategy's parameters (strategy-params.js): the fallback for a missing parameter, and the warm-up
const FrontendCalcParams = (typeof window !== 'undefined' && window.StrategyParams)
...
function backtestWithActiveDetection(dates, prices, dti, sevenDayDTIData, params = {}) {
    ...
    // A parameter the caller leaves out falls back to the strategy's (lib/shared/strategy-params.js)
    const tradingParams = {
        entryThreshold: params.entryThreshold || FrontendCalcParams.ENTRY_THRESHOLD,
        takeProfitPercent: params.takeProfitPercent || FrontendCalcParams.TAKE_PROFIT_PERCENT,
    ...
    earliestAllowableDate.setMonth(firstDate.getMonth() + FrontendCalcParams.WARMUP_MONTHS);
...
function checkForOpportunity(stockData, params = {}) {
    ...
    const entryThreshold = params.entryThreshold || FrontendCalcParams.ENTRY_THRESHOLD;
