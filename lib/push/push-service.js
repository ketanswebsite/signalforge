...
const { isAllowedPushEndpoint } = require('./endpoint-policy');
...
class PushService {
    ...
    async sendNotification(subscription, payload) {
        ...
        // Only a browser push service: rows saved before 2026-09-24 could name any URL at all
        if (!isAllowedPushEndpoint(subscription.endpoint)) {
            let host = 'an unparseable URL';
            try { host = new URL(subscription.endpoint).hostname; } catch (error) { /* keep the placeholder */ }
            console.warn(`[PUSH] Not sending to ${host}: not a push service; the subscription is switched off`);
