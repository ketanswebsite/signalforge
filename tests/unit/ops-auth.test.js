 ...
 * probes and triggers, the manual scan, the AI routine's feed). ANALYSIS_READ_TOKEN opens the six read-only
...
describe('server.js wiring', () => {
    ...
    test('control: the routes are found', () => {
        expect(TOKEN_ROUTES.length).toBe(13);
    ...
    test('the read token opens exactly the six read-only GET /api/ops probes', () => {
        expect(names(TOKEN_ROUTES.filter(r => /read: true/.test(r.rest)))).toEqual([
            ...
            'GET /api/ops/schedule-stats', 'GET /api/ops/sessions-stats', 'GET /api/ops/version'
        ]);
    });
