...
│   ├── shared/               backtest engines, DTI calculator, the Yahoo client (yahoo-client.js), price-unit and stale-fill repairs, stock universe (stock-data.js), date formats, process guards, input checks, the Postgres session store, account deletion (both delete routes), the admin SQL console's read-only runner
