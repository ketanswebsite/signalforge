...
| Endpoint test harness (every route, every page contract) | 1.2 | 2026-09-24 · endpoint | ✅ Working: 207 specs (128 live routes, 3 unmounted Stripe routes, 76 removed routes), 891 cases, all passing; no known bug is pinned. Two checks since 2026-09-24: `csv` (the content type) and `noCors` (no Access-Control-Allow-Origin header). The run ends with a reconcile dry run that must show zero ledger drift, available capital included, with the nightly check idle. Fresh scratch database and server per run, cron stubbed, no network. |
