...
| Endpoint test harness (every route, every page contract) | 1.0 | 2026-09-24 · endpoint | ✅ Working: 206 specs (144 live routes, 4 unmounted Stripe routes, 58 removed routes), 848 cases; 840 pass and 8 known bugs fail as expected. Fresh scratch database and server per run, cron stubbed, no network. |
...
**2026-09-24**
- *(this commit)* Admin money routes answer truthfully: a refund is recorded in one transaction (it always failed with 500, writing columns the payments table does not have), verify decides only a pending payment (404 for an unknown one, 422 for one already decided, where it used to answer success), and deleting a plan that any subscription still uses answers 409. 8 known bugs remain
