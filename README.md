...
| Simulator (`portfolio-backtest.html`) | 1.1 | 2026-09-24 · endpoint (add "+ browser" once the I24 browser check passes) | ⚠️ Faulty in the page (the harness only sees its routes, which pass): exit-reason colours are assigned by position; it uses a different engine from the live scan (GAPS #7); a missing close books a NaN P/L. Fixed on 2026-09-24: a symbol with `&` (M&M.NS, J&KBANK.NS, ...) loaded another company's history (the symbol went into the URL unencoded). |
| Positions chart dialog | 1.1 | 2026-09-24 · endpoint (add "+ browser" once the I24 browser check passes) | ⚠️ Faulty in the page (its routes pass): dark-mode legend colours are hard-coded; a dead parameter path. Fixed on 2026-09-24: a symbol with `&` loaded another company's chart (the symbol went into the URL unencoded). |
...
| Endpoint test harness (every route, every page contract) | 1.2 | 2026-09-24 · endpoint | ✅ Working: 207 specs (129 live routes, 3 unmounted Stripe routes, 75 removed routes), 894 cases, all passing; no known bug is pinned. Two checks since 2026-09-24: `csv` (the content type) and `noCors` (no Access-Control-Allow-Origin header). The run ends with a reconcile dry run that must show zero ledger drift, available capital included, with the nightly check idle. Fresh scratch database and server per run, cron stubbed, no network. (Counts at base 6ad7b61 were 145 live, 59 removed, 872 cases with 4 pinned bugs; if other batches land first, apply the delta: live −16, removed +16, cases +15, pinned bugs −4.) Keep the rest of the row as it is on main. |
