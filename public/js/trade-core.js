...
const TradeCore = (function() {
     ...
     */
    /**
     * Get market status using robust timezone calculations
     * Delegates to RobustMarketStatus module if available, falls back to basic logic
     * @param {string} symbol - Stock symbol
     * @returns {Object} - Market status object
     */
    function getMarketStatus(symbol) {
        // Use robust market status module if available
        if (typeof window.RobustMarketStatus !== 'undefined') {
            try {
                return window.RobustMarketStatus.getMarketStatus(symbol);
            } catch (error) {
                console.error('Error using RobustMarketStatus, falling back:', error);
            }
        }

        // Fallback: Simple status (always show as closed if robust module not loaded)
        console.warn('RobustMarketStatus module not loaded, using fallback status');
        return {
            status: 'unknown',
            statusText: 'Loading...',
            marketName: 'Market',
